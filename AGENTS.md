...
- Page families, each with its own layout on top of `Base`:
  ...
  - A CV like this — `LikeThisPage.astro`, used by `/cv/like-this/` and `/it/cv/like-this/`, shows the prompt in `src/data/like-this.ts` with a copy button; the home and CV footers link to it. The prompt describes this repository's structure: keep it in step when that structure changes.
  - In the data files every text is a `{ en, it }` pair (built with `t()`), so both languages change together. `LangNav.astro` is the EN/IT switch; each layout passes the paths of its two versions, which also become the `hreflang` alternates.
